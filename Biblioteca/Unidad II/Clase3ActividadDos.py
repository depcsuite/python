...
print("¿Desea crear un nuevo pedido antes de comenzar (S/N)?")
r = input("Ingrese su selección")
if r == "S":
    codigo = int(input("Codigo..."))
    nombre = input("Nombre...")
    precio = float(input("Precio..."))
    datosNuevo = (nombre, precio)
    datos[codigo] = datosNuevo    
...
while(codigo!=-1) :
    ...
    while(codigo < -1):
        print("ERROR; no existen códigos negativos. Ingrese otro...")
        codigo = int(input("¿Qué código desea agregar al pedido? (Ingrese -1 para finalizar)"))
    if(codigo!=-1):
        if codigo not in datos.keys():
            print("Este código no existe. Crearemos el producto.")
